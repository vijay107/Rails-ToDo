"""Drop-in replacement for the team's JiraAPI — talks to Frappe.

Updated for the new JSON response shape on get_issue / add_task_comment:
...
class FrappeAPI:
	...
	def get_issue(self, issue_id: str):
		try:
			data = self._call("jira_clone.jira_dev_integrations.get_issue", issue_name=issue_id)
			if not data.get("ok"):
	...
	def add_comment(self, issue_id: str, message: str):
		try:
			data = self._call("jira_clone.jira_dev_integrations.add_task_comment",
				task_name=issue_id, message=message)
			if not data.get("ok"):
				return True, data.get("error") or "unknown error"
			return False, data.get("message") or "comment created"
		except Exception as e:
	...
	def validate_merge_condition_and_get_prlist(self, issue_list, merger_user_email: str, blank_deployment: bool = False):
		try:
			errored, payload, target = self._call(
				"jira_clone.jira_dev_integrations.validate_merge_condition_and_get_prlist",
...
def main():
	"""CLI entry — used by the Jenkinsfile."""
	import os
	cmd = sys.argv[1]
	...
	if cmd == "validate":
		...
		print(json.dumps({"errored": err, "payload": prs_or_msg, "target_branch": target}))
